...
test('portal and admin report issue buttons share one feedback page', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /登录工作台/ }).click()
  await expect(page).toHaveURL(/\/admin$/)
  await clickAdminNavigation(page, '问题报告')
  await expect(page).toHaveURL(/\/admin\/feedback/)
  const submittedCard = page.locator('.feedback-item-card').filter({ hasText: '希望增加活动日历' })
  await expect(submittedCard).toBeVisible()
  await expect(submittedCard).toContainText('门户首页可以按月份查看即将开始的活动。')
  await expect(submittedCard).toContainText('林沐')
  await expect(submittedCard).toContainText('linmu@example.com')
  await submittedCard.getByRole('button', { name: '标记已处理' }).click()
  await expect(submittedCard.getByText('已处理', { exact: true })).toBeVisible()
...
test('activity planner opens as a structured three-step workspace', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: /自定义门户接入约定/ })).toBeVisible()
  await page.getByPlaceholder('搜索活动规范、场地要求或复盘关键词；留空查看最近资料').fill('门户 不存在的补充词')
