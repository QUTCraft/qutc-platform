...
import { ArrowLeft, DataAnalysis, Document, Monitor, Setting, SwitchButton, UserFilled } from '@element-plus/icons-vue'
...
<template>
  ...
  <div class="admin-app">
    <aside class="admin-rail" :class="{ 'is-open': menuOpen }">
      ...
      <div class="rail-footer"><RouterLink to="/" class="portal-link"><el-icon><ArrowLeft /></el-icon>返回门户</RouterLink><div class="account-chip"><span>{{ session.user?.display_name.slice(0, 1) ?? 'Q' }}</span><div><strong>{{ session.user?.display_name ?? '未登录' }}</strong><small>{{ roleLabel }}</small></div><el-button text circle aria-label="退出登录" @click="logout"><el-icon><SwitchButton /></el-icon></el-button></div></div>
    </aside>
