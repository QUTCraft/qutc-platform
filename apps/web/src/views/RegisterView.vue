  ...
  <div class="register-page">
    <section class="register-card">
      ...
      <AsyncState v-if="invitationToken" :loading="loading" :error="error" @retry="refresh">
        <template v-if="invitation"><p class="register-context">你正在接受 {{ invitation.organization_name }} 的成员邀请，请使用 {{ invitation.email }} 注册。</p></template>
      </AsyncState>
      <h1>创建成员账户</h1>
      <el-form ref="formRef" :model="form" :rules="rules" label-position="top" size="large" @submit.prevent="submit">
        <el-form-item label="邮箱" prop="email"><el-input v-model="form.email" type="email" autocomplete="email" placeholder="name@example.com" /></el-form-item>
        ...
        <el-button type="primary" native-type="submit" :loading="submitting" round>注册并继续</el-button>
      </el-form>
