...
> 状态：AI-0、AI-1 与服务创新类活动策划、质量评估、领域专用人工批准已实现；AI-2、AI-3 与通用 AI-4 延期
> 目标版本：`v0.1.0-competition`（当前比赛版实现基线）
> 更新日期：2026-08-05
> 已满足前置条件：RBAC、审计、内容状态机、知识可见性和 API 契约均已接入真实实现
...
AI 不成为权限系统或审批系统的替代品。它是现有 CMS 的受限调用者。
...
| `external_high_risk` | 邮件群发、Webhook、外部管理动作 | 第一阶段禁止。 |
...
当前已有 14 条后台 AI 接口进入 Go、OpenAPI、Apifox 和 TypeScript client，覆盖组织配置、智能体目录、持久运行、知识检索、活动方案、人工评分和领域专用批准。面向任意智能体的通用工具批准/拒绝与公开问答仍为规划：
...
创建响应返回完整运行摘要，但不会假装同步完成。客户端通过运行查询接口轮询终态：
...
比赛版部署在同一服务器，AI worker 作为 API 进程内的单机后台任务运行，任务队列持久化在 MySQL：

```text
Web
API（HTTP + 单机 Agent Worker）
MySQL（AgentRun 队列、引用快照与业务数据）
Redis（现有缓存能力，不承担当前 AI 队列）
Object Storage（媒体文件）
```
...
- ✅ 10 组校园活动场景、Prompt Injection、引用与真实模型稳定性基线已建立；更大规模对抗数据集和持续回归继续在 AI-2 完善。
...
- ✅ 活动策划已实现领域专用的低风险建议操作、逐项人工选择，以及事务创建非公开项目、里程碑和公告草稿；
- ⏳ 面向任意智能体的通用 ToolCall/AgentApproval 状态机、通用批准/拒绝接口、失败重试和补偿延期到 v0.2；
- 即使后续扩展，也不开放任意外部命令、角色修改、成员审批决定和自动发布。

## 13. MVP 建议

比赛主展示采用一个更贴合“服务创新类”的窄而完整闭环：

```text
运营者填写活动目标、受众、时间、预算和约束
→ 显式选择组织知识资料
→ AI 返回带固定引用的结构化活动方案
→ 运营者审阅方案并完成五维质量评分
→ 人工逐项选择建议操作
→ 服务端事务创建非公开项目、里程碑和公告草稿
→ 负责人继续编辑，按既有权限发布到门户
```

内容协作的“知识 → Markdown → 对比 → 人工创建草稿 → 发布”闭环保留为第二演示路径，用来证明同一智能体底座可以复用到 CMS 编辑工作。
