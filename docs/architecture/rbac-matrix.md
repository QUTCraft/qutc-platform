...
- `editor` 可创建内容，但只能查看、编辑、恢复、上传附件或提交自己创建的 `draft`/`review`/`archived` 内容；同级编辑者不能查看或修改他人的未发布与待审核文章。已发布内容必须由原作者申请下线后再编辑。
- `member` 只有 `organization:read`，不能读取申请审批、内容审核备注或他人草稿；工作台不得返回待处理申请列表。
