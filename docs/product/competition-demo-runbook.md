...
同时完成一次全新数据库迁移和一次旧数据卷升级；确认 `schema_migrations` 从 001 连续到 018，既有内容已由 015 幂等补齐 v1 修订基线，媒体资产已由 018 补齐外部提供方字段。最终答辩前查看 [已知限制](../operations/known-issues.md)，确保口径与实际实现一致。
