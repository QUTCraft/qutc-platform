...
10. 时间允许时展示多组织切换、批量邀请与组织模板、申请审批通知队列、内容修订恢复、资源下载统计、自定义 Portal 回退和 QUTCraft ServerAdapter，作为平台完整度而非 AI 主线。
...
同时完成一次全新数据库迁移和一次旧数据卷升级；确认 `schema_migrations` 从 001 连续到 015，既有内容已由 015 幂等补齐 v1 修订基线。最终答辩前查看 [已知限制](../operations/known-issues.md)，确保口径与实际实现一致。
