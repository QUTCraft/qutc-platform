...
| 真实 API 连续主演示演练 | `openai_compatible` / `real` | 3 / 3 | 13,193 ms | 21,558 ms | 2,196 / 4,675 |

真实模型结果通过的是上述自动结构与安全检查。正式比赛演示前应在活动策划页的“待我评分”队列中恢复方案并保存至少一轮五维人工评分，再连续运行三轮主演示，避免把一次成功误认为稳定性结论。历史方案入口属于活动策划工作台内部，不作为独立后台模块占用侧边栏。

## 5. 真实 API 连续演练

自动数据集直接验证模型驱动；比赛主演示还需要验证登录、组织配置、知识检索、活动任务创建、异步轮询和 API 响应契约。运行：

```powershell
.\scripts\run-ai-activity-demo-rehearsal.ps1 -Rounds 3
```

脚本默认拒绝 Mock，连续三轮调用当前 Compose API，并核验 `ready`、真实模型模式、`activity-planner/v2`、固定引用、六类建议操作、Markdown 标题和来源 URI。脱敏报告只保存模型标识、Prompt 版本、延迟、Token、计数、请求/方案 ID 和归一化失败类别；`human_evaluation` 与 `human_approval` 固定标记为 `pending`，必须由真实操作者在管理端完成。

2026-08-02 的首轮真实 API 演练已达到 3/3，三轮耗时分别为 21,558 ms、8,654 ms 和 9,366 ms；三个方案保留在当前组织的“历史方案”中，供团队完成真实人工评分和至少一次建议批准。报告位于本机被忽略的 `tmp/agent-rehearsal`，不进入版本库。
