...
> 当前仓库处于比赛版收口阶段。前端支持契约 Mock 与远程 API 两种模式；Go API、001—020 版本化迁移、JWT/RBAC、Compose 环境，以及内容审核、资源、项目、预创建成员邀请、申请审核和 AI 活动策划闭环均已落地。当前重点是新机器复现、视觉/故障回归和交付冻结。
...
- 内容工作区：标准 Markdown 编辑、实时预览、Markdown 导入、图片/附件插入、作者级编辑边界、提交审核/退回/发布/申请下线闭环，以及带编辑者与差异统计的不可变修订历史；扩展语法见 [Markdown 内容编写说明](docs/product/markdown-authoring.md)。
...
- 组织公开资料：维护名称、简称、标语、介绍、官网 Logo、备案号、公开邮箱、社交链接和公开状态，修改立即作用于门户并留存审计。
...
脚本读取本机 `deploy/compose/.env`，不会输出管理员密码，并以 `integration` 构建标签运行真实 API、MySQL 与 Redis 测试。测试连续执行三轮“草稿不可见 → 提交审核仍不可见 → 管理员发布可见 → 重复发布冲突 → 下线不可见”，并额外验证同级编辑者不能修改/提交他人文章、审核中不可改稿、原作者申请下线后可重新编辑；同时确认 Portal 缓存及时失效。资源用例覆盖草稿、跨组织、已发布和已下线四种下载边界。
